defmodule Plus2048.GameServer do
  use GenServer
  alias Tty2048.Game

  @game_modes [:anarchy, :democracy]
  @directions [:up, :down, :left, :right]
  @peroid if Mix.env() == :test, do: 10, else: 5_000
  ...
  def toggle_game_mode(game_mode) when game_mode in @game_modes do
    GenServer.call(__MODULE__, {:toggle_game_mode, game_mode})
  end
    ...
    case state.game_mode do
      ...
      :democracy ->
        ...
        if !state.timer_ref do
          ...
          timer = Process.send_after(self(), {:make_next_turn, from}, @peroid)

          voting_ends_at = DateTime.utc_now |> DateTime.add(@peroid, :millisecond)
  ...
  def handle_call({:toggle_game_mode, game_mode}, _from, %GameState{} = state) do
    cancel_timer(state.timer_ref)
  ...
  defp sanitize(%GameState{} = state) do
    %{game: {status, grid_and_score}} = state
